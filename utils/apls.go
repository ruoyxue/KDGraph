// Reference: https://github.com/songtaohe/Sat2Graph
...
var min_distance_filter float64 = 15.0 // don't consider shortest path shorter than 100.0
var prop_step int = 3 // one-to-one matching, if two control points mapped to A,B in the inferred graph, distance between A and B on the graph should greater than '4' hops, each hop is 2 meters.

var region_size float64 = 1300.0 // unit is meter
var margin_size float64 = 60.0 // unit is meter 
var match_distance float64 = 35.0 // max distance for two nodes to be matched
...
func apls_one_way(graph_gt *graph, graph_prop *graph, ret chan float64) {
	...
	for nid1, _ := range control_point_gt {
		...
		for _, result := range results {
			...
			if GPSDistance(result.(*gpsnode).loc, graph_gt.Nodes[nid1]) < match_distance {
...
func apls(graph_gt *graph, graph_prop *graph) float64 {
    ...
    return (apls_gt+apls_prop)/2.0

}

//export compute
func compute(path1  * C.char, path2  * C.char) C.float {
	graph_gt := LoadGraphFromJson(C.GoString(path1))
	graph_prop := LoadGraphFromJson(C.GoString(path2))
	...
	result := apls(graph_gt_dense, graph_prop_dense)
